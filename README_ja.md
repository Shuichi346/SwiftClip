...
SwiftClipは、macOS 26.0以降を対象としたローカルファースト設計のmacOSメニューバー型クリップボードマネージャー兼スニペットランチャーです。ClipyのUIおよび使いやすさにインスパイアされており、クリップボード履歴、再利用可能なスニペット、設定、インポート／エクスポートデータをすべてユーザーのMac上で管理します。アクセシビリティ権限が付与されると、選択した履歴またはスニペット項目を直前にフォーカスしていたアプリに貼り付けることができます。Swift 6およびSwiftUIで構築されており、Clipy互換のXMLインポート／エクスポート、グローバルショートカット、フォーマットフィルタリングをサポートしています。
